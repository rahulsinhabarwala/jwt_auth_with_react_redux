    ...
    if(localStorage.userDataList){
        ...
        if(item.userName === this.state.userName && item.userPassword === this.state.userPassword){
          userExist = true
           ...
           this.setState({errorMessage:"Entered data not correct"}) 
        }
      })
        if(userExist){
          let token = jwt.sign({
            ...
            <form className="form-horizontal">
                <div className="form-group">
                    ...
                    <div className="col-sm-10">
                        <input type="name" className="form-control" name='userName' id="email" placeholder="Enter name" value={this.state.userName} onChange={this.onChange} />
                    </div>
                 </div>
                  <div className="form-group">
